...
export class GLTFLoader {
    ...
    async loadOBB(url) {
        this.obb = await this.fetchJson(url);
    }
    ...
    async loadLight(nameOrIndex) {
        ...
        if (gltfSpec.type === 'spot') {
            const light = new SpotLight(gltfSpec);
            this.cache.set(gltfSpec, light);
            return light;
        }
        else if(gltfSpec.type === 'directional') {
            const light = new DirectionalLight(gltfSpec);
            this.cache.set(gltfSpec, light);
            return light;
        }
    }
    ...
    async loadNode(nameOrIndex) {
        ...
        const nodeName = gltfSpec.name;
        const objectName = nodeName.replace(/\.\d+$/, '');
        const obb = new OBB(this.obb[objectName]);
        
        const options = { ...gltfSpec, children: [], name: nodeName, obb: obb };
        ...
        if (gltfSpec.extensions !== undefined && gltfSpec.extensions.KHR_lights_punctual !== undefined && gltfSpec.extensions.KHR_lights_punctual.light !== undefined) {
            options.light = await this.loadLight(gltfSpec.extensions.KHR_lights_punctual.light);
        }
        if(gltfSpec.extras !== undefined && gltfSpec.extras.type === 'Car') {
            options.car = new Car(gltfSpec.extras);
        }
        if(gltfSpec.extras !== undefined && gltfSpec.extras.userRotation === 'true') {
            options.customRotation = new Rotation(Number(gltfSpec.extras.yaw), Number(gltfSpec.extras.pitch));
        }
